...
async fn main() {
    ...
    dub_line(
        &dub_config.dubber_config,
        &"./test/".to_string(),
        &"No tendríamos este problema si no hubieras destrozado mi coche oficial.".to_string(),
        &"13_test.wav".to_string(),
    )
    .await;
}
